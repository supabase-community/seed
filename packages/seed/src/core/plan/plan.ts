  ...
  private async generateModel(
    ...
    for (let index = 0; index < modelsInputs.length; index++) {
      ...
      const handleScalarField = async (field: DataModelScalarField) => {
        ...
        const userModelsOverride = userModels[model].data?.[field.name];
        // if the model override is coming from the user this will be false, otherwise (codegen) it'll be true
        const isFallbackModelOverride =
          typeof userModelsOverride === "function" &&
          userModelsOverride[FallbackSymbol] === true;
    ...
    if (this.connectStore) {
      for (const modelName of Object.keys(this.connectStore)) {
        if (this.connectStore[modelName].length > 0) {
          ...
          connectFallback[FallbackSymbol] = true;
