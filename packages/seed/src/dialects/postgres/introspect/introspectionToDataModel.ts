...
function getParentRelationAndFieldName({
    ...
    const isMultiple =
        table.parents.filter(
            (p) => p.targetTable === parentRelation.targetTable,
        ).length > 1;
...
function getChildRelationAndFieldName({
    ...
    const isMultiple =
        table.children.filter((c) => c.fkTable === childRelation.fkTable)
            .length > 1;
    const relationName = isMultiple
...
const computeEnumType = (
    ...
    const matchingEnum = introspection.enums.find(
        (e) => e.id === column.typeId,
    );
    ...
    for (const table of introspection.tables) {
        ...
        for (const parentRelation of table.parents) {
            ...
            const field: DataModelField = {
                ...
                relationToFields: parentRelation.keys.map(
                    (k) => k.targetColumn,
                ),
                isList: false,
                isId: false,
